            ...
            <div className="cell">
            {
                hoverNumbers.map(number => {
                    const hoverAreaClasses = clsx("hoverArea", {
                        lightPencilMark: this.state.hover.isHovering && this.state.hover.value === number, 
                        darkPencilMark: this.state.pencilMarks.includes(number)
                    });
                    return <div className={hoverAreaClasses} onMouseEnter={() => this.handleMouseHover(number, true)} onMouseLeave={() => this.handleMouseHover(number, false)}>{number}</div>
                })
            }
            </div>
        )
